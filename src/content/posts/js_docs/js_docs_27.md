...
因为 `instanceof` 操作符是判断原型链上的对象,而当一个对象并不是通过原型构造出来的时候(直接 return 一个对象),此方法就失效了.
...
在很早之前,使用的 32 位系统,为了性能考虑使用低位存储了变量的类型信息.
...
此外还有 2 个比较特殊的指: `undefined` 和 `null`

- undefined 用 (-2^30) 表示
- null 对应机器码的 NULL 指针,一般是全零

这样一来,`null` 就出了一个 bug.因为它对应的空指针,低位是 000,因此被判断成一个对象了.且这个 bug,应该是永远都不会去修复了.
...
在 ES2015 之前,`typeof` 总能保证对任何所给的操作数返回一个字符串,即使是未声明的标识符,也能返回`undefined`. 但是在有了`let` 和 `const` 块级作用域后,在其被声明之前对块中的`let` 和 `const` 变量使用 `typeof` 将会抛出一个 `ReferenceError` .
