...
`radix`可选,表示要解析的数字的基数,介于 2 ~ 36 之间,若省略该参数或该参数为 0,则为 10.若要解析的数字以`0x`或`0X`开头,则将以 16 为基数
...
`1.toString()`为什么会报错?
因为 JS 引擎无法确定这里的`.`是什么意思,是`浮点数`还是`点运算符`?
...
其实除了 forEach,数组的其他迭代方法如:`every()`,`some()`,`find()`,`map()`,`filter()`等都有可选的第二参数

## console.log()

console.log()支持占位符,比如:字符(%s) ,整数(%d 或%i),浮点数(%f),样式(%c)等.第二个参数开始(可能会有第三个第四个)填入的是要去填坑占位符的内容或样式
